...
class iSellBase extends ProcessorBase{
    ...
    public function selectPassiveCompany( $passive_company_id ){
        ...
        if( !$this->checkAssignedPassiveCompany( $passive_company_id ) )
            return;
        $this->_pcomp=$_SESSION['pcomp']=$passive_company_id?$this->get_row("SELECT * FROM companies_list LEFT JOIN curr_list USING(curr_code) WHERE company_id='$passive_company_id'"):NULL;
    }
