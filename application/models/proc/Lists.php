...
class Lists extends Data {
	...
	$this->Base->query("UPDATE event_list SET $set WHERE event_id='$event_id'");
    }

    public function addEvent() {
	$user_id = $this->Base->svar('user_id');
	$this->Base->query("INSERT INTO event_list SET event_date=NOW(),event_user_id=$user_id");
	...
	return $this->Base->get_list("SELECT event_label,COUNT(event_label) AS count FROM event_list WHERE event_status<1 AND DATE(event_date)='$selected_day' AND IF(event_is_private,IF(event_user_id='$user_id' OR $user_level=3,1,0),1) GROUP BY event_label");
    }

    public function getEventDates() {//must be optimized
	$user_id = $this->Base->svar('user_id');
	$user_level = $this->Base->svar('user_level');
	return $this->Base->get_list("SELECT DATE_FORMAT(event_date,'%Y-%m-%d') AS event_date FROM event_list WHERE event_status<1 AND IF(event_is_private,IF(event_user_id='$user_id' OR $user_level=3,1,0),1) GROUP BY event_date");
    }
