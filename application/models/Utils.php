...
class Utils extends Data {
	...
	if ($action == 'download') {
	    return $this->updateDownload(BAY_UPDATE_URL,'E:\update.zip');
	}
	if ($action == 'install') {
	    return $this->updateInstall('E:\update.zip');
	}
    }
	...
	if ($update === false) {
	    $this->Base->msg('Could not download update `' . $updateUrl . '`!');
	    return false;
	}
	$handle = fopen($updateFile, 'w');
	if (!$handle) {
	    $this->Base->msg('Could not save update file `' . $updateFile . '`!');
	    return false;
	}
	if (!fwrite($handle, $update)) {
	    $this->Base->msg('Could not write to update file `' . $updateFile . '`!');
	    return false;
	}
	fclose($handle);
	return true;
    ...
    public function updateInstall($updateFile) {
	$this->installDir='E:\update';
	$this->dirPermissions='0777';
	$this->updateScriptName='';
	$this->removeTempDir=false;
	
	$zip = zip_open($updateFile);
	while ($file = zip_read($zip)) {
	    $filename = zip_entry_name($file);
	    $foldername = $this->installDir . dirname($filename);
	    $this->Base->msg('Updating `' . $filename . '`!');

	    if (!is_dir($foldername)) {
		if (!mkdir($foldername, $this->dirPermissions, true)) {
		    $this->Base->msg('Could not create folder `' . $foldername . '`!');
		}
	    }
		...
		if (!is_writable($this->installDir . $filename)) {
		    $this->Base->msg('Could not update `' . $this->installDir . $filename . '`, not writable!');
		    return false;
		}
	    } else {
		$this->Base->msg('The file `' . $this->installDir . $filename . '`, does not exist!');
		$new_file = fopen($this->installDir . $filename, "w") or $this->Base->msg('The file `' . $this->installDir . $filename . '`, could not be created!');
		fclose($new_file);
		$this->Base->msg('The file `' . $this->installDir . $filename . '`, was succesfully created.');
	    }
	    $updateHandle = @fopen($this->installDir . $filename, 'w');
	    if (!$updateHandle) {
		$this->Base->msg('Could not update file `' . $this->installDir . $filename . '`!');
		return false;
	    }
	    if (!fwrite($updateHandle, $contents)) {
		$this->Base->msg('Could not write to file `' . $this->installDir . $filename . '`!');
		return false;
	    }
	    fclose($updateHandle);
	    //If file is a update script, include
	    if ($filename == $this->updateScriptName) {
		$this->Base->msg('Try to include update script `' . $this->installDir . $filename . '`.');
		require($this->installDir . $filename);
		$this->Base->msg('Update script `' . $this->installDir . $filename . '` included!');
		unlink($this->installDir . $filename);
	    }
	}
	zip_close($zip);
	if ($this->removeTempDir) {
	    $this->Base->msg('Temporary directory `' . $this->tempDir . '` deleted.');
